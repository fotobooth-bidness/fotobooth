...
### Digital copies
Controls the "Get digital copy" button on the result screen.
- **Auto** — shows the button only when the tablet has an internet connection; hides it when offline. Good default.
- **On** — always shows the button.
- **Off** — hides it entirely: a clean, print-only booth for offline events.
...
**Important — how delivery works today vs. later:** right now sharing happens from *the tablet's* share sheet, so it sends to apps/accounts on the tablet, not wirelessly to a guest's phone. True "scan the QR → download your own photos" delivery to any phone (iPhone or Android) needs the tablet to be online and the photos hosted somewhere — that upload + gallery piece is planned for when you add a hotspot. The app is structured so that path can be added without reworking the rest.
